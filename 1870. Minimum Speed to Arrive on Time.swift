class Solution {
    ...
    // go through 1...10^7 to find proper speed to let reach on hour
    ...
    func minSpeedOnTime(_ dist: [Int], _ hour: Double) -> Int {
        let n = dist.count
        let intHour = hour.truncatingRemainder(dividingBy: 1) != 0 ? Int(hour + 1) : Int(hour)
        if intHour < n { return -1 }
        var left = 1
        var right = Int(1e7)

        while left < right {
            let mid = left + (right - left) / 2
            let h = getHour(dist, n, mid)
            if h > hour {
                left = mid + 1
            } else {
                right = mid
