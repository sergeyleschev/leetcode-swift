...
class Solution {
    ...
    func isRationalEqual(_ s: String, _ t: String) -> Bool {
        stringToFraction(s).isEqualTo(other: stringToFraction(t))
    }
    ...
    private func integerToFraction(_ integer: String) -> Fraction {
        Fraction(numerator: Int(integer)!, denominator: 1)
    }
    ...
    private func doubleToFraction(_ numStr: String) -> Fraction {
        ...
        return Fraction(
            numerator: integer * powerOfTen[floatLength] + Int(floatStr)!,
            denominator: powerOfTen[floatLength])
    }
    ...
    private func circulatingDecimalToFraction(_ numStr: String) -> Fraction {
        ...
        let repeatingPart = String(
            numStr[numStr.index(after: leftParenthesisIdx)..<rightParenthesisIdx])
        return Fraction(
            numerator: powerOfTen[repeatingPart.count + nonRepeatingPart.count] * integer + Int(
                nonRepeatingPart + repeatingPart)!
                - (powerOfTen[nonRepeatingPart.count] * integer + (Int(nonRepeatingPart) ?? 0)),
            denominator: powerOfTen[nonRepeatingPart.count + repeatingPart.count]
                - powerOfTen[nonRepeatingPart.count])
    }
