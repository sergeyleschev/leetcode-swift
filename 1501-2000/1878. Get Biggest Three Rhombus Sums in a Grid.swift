    ...
    func getBiggestThree(_ grid: [[Int]]) -> [Int] {
        ...
        func getSum(_ r: Int, _ c: Int, _ length: Int) -> Int? {
            ...
            for delta in 0..<length {
                sum +=
                    grid[r + delta][c - delta] + grid[r + delta][c + delta]
                    + grid[maxRow - delta][c + delta] + grid[maxRow - delta][c - delta]
            }
            sum -=
                (grid[r][c] + grid[maxRow][c] + grid[r + length - 1][c - length + 1]
                    + grid[r + length - 1][c + length - 1])
            return sum
        }
