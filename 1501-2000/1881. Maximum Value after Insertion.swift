class Solution {
    ...
    func maxValue(_ n: String, _ x: Int) -> String {
        ...
        let xStr = "\(x)".first!
        if first == "-" { return getAnsNegative(n, xStr) }
    ...
    private func getAnsPositive(_ n: String, _ xStr: Character) -> String {
        ...
        if let idx = n.firstIndex(where: { $0 < xStr}) { ans.insert(xStr, at: idx) } 
        else { ans.append(xStr) }
    ...
    private func getAnsNegative(_ n: String, _ xStr: Character) -> String {
        ...
        if let idx = ans.firstIndex(where: { $0 > xStr && $0 != "-"}) { ans.insert(xStr, at: idx) } 
        else { ans.append(xStr) }
