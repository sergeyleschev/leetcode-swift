class Solution {
    ...
    func cherryPickup(_ grid: [[Int]]) -> Int {
        ...
        var n = grid[0].count
        var dpCache = [[[Int]]](
            repeating: [[Int]](repeating: [Int](repeating: -1, count: n), count: n), count: m)

        return dp(row: 0, col1: 0, col2: n - 1, grid: grid, n: n, m: m, dpCache: &dpCache)
    }

    private func dp(
        row: Int, col1: Int, col2: Int, grid: [[Int]], n: Int, m: Int, dpCache: inout [[[Int]]]
    ) -> Int {
            ...
            for newCol1 in (col1 - 1)...(col1 + 1) {
                for newCol2 in (col2 - 1)...(col2 + 1) {
                    cur = max(
                        cur,
                        dp(
                            row: row + 1, col1: newCol1, col2: newCol2, grid: grid, n: n, m: m,
                            dpCache: &dpCache))
                }
            }
            ans += cur
