...
class MyCalendarTwo {
    ...
    // - Complexity:
    //   - time: O(N^2), where NN is the number of events booked. For each new event, we process every previous event to decide whether the new event can be booked. This leads to \sum_k^N O(k) = O(N^2) complexity.
    //   - space: O(N)O(N), the size of the calendar.
